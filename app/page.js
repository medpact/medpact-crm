...
const {data:reqHosp} = await supabase
.from("requirements")
.select("hospital_id")
...
{specialtyChart.datasets?.length > 0 && (
  <Bar data={specialtyChart}/>
)}
</div>
...
{hospitalChart.datasets?.length > 0 && (
  <Bar data={hospitalChart}/>
)}
</div>
