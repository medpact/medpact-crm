...
/**
 * MedPact — Medical Tourism landing page
 * Drop-in replacement for app/medicaltourism/page.js
 *
 * Data: same Supabase tables as before (procedures, testimonials, doctors).
 * Everything falls back to sensible sample content if a table is empty,
 * so the page never renders half-blank while you fill the database.
 *
 * Optional extra table for the enquiry form:
 *   create table leads (
 *     id uuid primary key default gen_random_uuid(),
 *     name text, email text, country text, phone text,
 *     procedure text, message text,
...
const SAMPLE_PROCEDURES = [
  { id: "s1", name: "Heart valve replacement", price_india: 9400, price_us: 170000, category: "Cardiac", stay_days: 21 },
  { id: "s2", name: "Coronary artery bypass (CABG)", price_india: 7200, price_us: 123000, category: "Cardiac", stay_days: 18 },
  { id: "s3", name: "Full-mouth dental implants", price_india: 6500, price_us: 48000, category: "Dental", stay_days: 12 },
  { id: "s4", name: "Spinal fusion", price_india: 8800, price_us: 110000, category: "Neuro", stay_days: 16 },
  { id: "s5", name: "Total knee replacement", price_india: 6200, price_us: 52000, category: "Orthopaedic", stay_days: 14 },
]
...
function Specialities({ procedures, currency }) {
  const groups = useMemo(() => {
    ...
    return Object.entries(byCat).slice(0, 4)
  }, [procedures])
...
function Comparison({ procedures, currency }) {
  ...
  const rows = procedures.slice(0, 5)
  const ceiling = Math.max(...rows.map((p) => p.price_us))
    ...
    <section id="prices" className="band band-ink" ref={ref}>
      <div className="shell">
        <SectionHead
          ...
          note="Average private-hospital price in the United States, against the MedPact package in India. Treatment only — travel is listed separately in your estimate."
        />

        <div className="chart">
          {rows.map((p) => {
            const usW = seen ? (p.price_us / ceiling) * 100 : 0
            const inW = seen ? (p.price_india / ceiling) * 100 : 0
            return (
              <div className="chart-row" key={p.id}>
                <div className="chart-label">
                  <span className="chart-name">{p.name}</span>
                  <span className="chart-cut">
                    {Math.round((1 - p.price_india / p.price_us) * 100)}% less
                  </span>
                </div>

                <div className="bars">
                  <div className="bar-line">
                    <span className="bar-key">USA</span>
                    <div className="track">
                      <div className="fill fill-us" style={{ width: usW + "%" }} />
                    </div>
                    <span className="bar-val">{money(p.price_us, currency)}</span>
                  </div>
                  <div className="bar-line">
                    <span className="bar-key">India</span>
                    <div className="track">
                      <div className="fill fill-in" style={{ width: inW + "%" }} />
                    </div>
                    <span className="bar-val">{money(p.price_india, currency)}</span>
                  </div>
                </div>
              </div>
            )
